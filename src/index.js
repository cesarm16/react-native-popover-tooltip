...
import PopoverTooltipItem, { type Label, labelPropType } from './PopoverTooltipItem'
...
class PopoverTooltip extends React.PureComponent<Props, State> {
	...
	toggleModal = () => {
		...
		if (this.state.isModalOpen && this.props.atClosed) this.props.atClosed()
	}
	...
	onInnerContainerLayout = (event: {
		...
		if (!this.state.willPopUp || tooltipContainerWidth === 0 || tooltipContainerHeight === 0) {
			return
		}
		...
		componentWrapper.measure((x, y, width, height, pageX, pageY) => {
			const fullWidth = pageX + tooltipContainerWidth + (width - tooltipContainerWidth) / 2
			const tooltipContainerX_final =
				fullWidth > window.width
					? window.width - tooltipContainerWidth
					: (window.width - tooltipContainerWidth) / 2
			let tooltipContainerY_final = this.state.tooltipTriangleDown
				? pageY - tooltipContainerHeight - SEPARATION
				: pageY + tooltipContainerHeight - SEPARATION
			let tooltipTriangleDown = this.state.tooltipTriangleDown
			if (pageY - tooltipContainerHeight - SEPARATION < 0) {
				tooltipContainerY_final = pageY + height + SEPARATION
				tooltipTriangleDown = false
			}
			if (pageY + tooltipContainerHeight + 80 > window.height) {
				tooltipContainerY_final = pageY - tooltipContainerHeight - SEPARATION
			...
			const tooltipContainerY = this.state.tooltipContainerScale.interpolate({
				inputRange: [0, 1],
				outputRange: [
					tooltipContainerY_final + tooltipContainerHeight / 2 + SEPARATION,
			...
			const tooltipTriangleLeftMargin = pageX + width / 2 - tooltipContainerX_final - 10
			this.setState(
				{
					x: pageX,
					y: pageY,
	...
	render() {
		...
		return (
			<TouchableOpacity
				...
				activeOpacity={1.0}>
				<Animated.View
					style={[{ opacity: this.state.oppositeOpacity }, this.props.componentContainerStyle]}>
				...
				<Modal
					...
					transparent>
					<Animated.View
						style={[styles.overlay, this.props.overlayStyle, { opacity: this.state.opacity }]}>
						<TouchableOpacity
							activeOpacity={1}
							focusedOpacity={1}
							style={styles.button}
							...
							<Animated.View
								...
								<View onLayout={this.onInnerContainerLayout} style={styles.innerContainer}>
									{triangleUp}
									<View
										style={[
